...
sequencia_file_path = '6_67.txt'  # Arquivo com as sequências
output_sh_path = '6_67.sh'        # Arquivo .sh a ser gerado
...
def gerar_range(sequencia):
    range_inicial = f"{sequencia}0000000000000"
    range_final = f"{sequencia}fffffffffffff"
...
if quantidade_linhas > len(linhas):
    print("Erro: O arquivo 6_67.txt não possui sequências suficientes.")
    exit()
...
with open(output_sh_path, 'w') as sh_file, open(historico_path, 'a') as historico_file:
    ...
    sh_file.write("echo \"Iniciando script 6_67.sh com proteção para Ctrl+C.\"\n\n")
    ...
    for i, sequencia in enumerate(sequencias_escolhidas, start=1):
        ...
        sh_file.write(f"timeout 300s ./keyhunt -m rmd160 -f tests/67.rmd -n 102400 -R -l compress -s 3 -t 24 -e -S {out_file_path} -r {range_completo} \n")
        sh_file.write("sleep 10\n")
    ...
    # Reiniciar o ciclo chamando cpu67.py
